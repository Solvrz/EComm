...
class Bag {
  ...
  void increaseQuantity(Product product, {int increase = 1}) {
    for (final bagItem in _products) {
  ...
  bool containsProduct(Product product) {
    bool? contains;
    for (final BagItem bagItem in _products) {
      if (bagItem.product == product) {
        contains = true;
        break;
      }
  ...
  Future<void> load() async {
    ...
    if (bagData != null) {
      final List<BagItem?> items = bagData
          .map<BagItem>(
        ...
        if (item != null) {
          ...
          if (products.docs.isEmpty) {
            ...
            final Map<dynamic, dynamic> productData =
                products.docs.first.data()! as Map;
            final List<String> diff = item.product.difference(
              Product.fromJson(productData),
            );

            final Map<dynamic, dynamic> updatedData = item.product.toJson();

            for (final changeKey in diff) {
              if (changeKey == "variations") {
                ...
                updatedData["selected"] =
                  ...
                  final Variation variation = Variation.fromJson(element);
                  return MapEntry(
                    variation.name,
